module Isdc.Ui.V3.Radio.List exposing (..)

import Html.Styled as Styled
import Isdc.Ui.V3.Radio.Radio as Radio exposing (Radio(..))
...
type Msg a
    = RadioClicked (Radio a)


update : Msg a -> RadioList a -> RadioList a
update msg radioList =
    case msg of
        RadioClicked radio ->
            select radio radioList
...
view : String -> (Msg a -> msg) -> RadioList a -> Styled.Html msg
view selectedId msg radioList =
    Styled.map msg (viewRadioList selectedId radioList)


viewRadioList : String -> RadioList a -> Styled.Html (Msg a)
viewRadioList selectedId selection =
    mapSelected
        { selected = Radio.selected selectedId
        , rest = Radio.unselected RadioClicked
        }
        selection
        |> toList
        |> Styled.div []
